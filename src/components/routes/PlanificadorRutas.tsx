...
import { peajesData } from "../../data/peajes";
...
export default function PlanificadorRutas() {
    ...
    const encontrarPeajesEnRuta = (coordenadas: Array<{ lat: number; lng: number }>, rutaKey: string): typeof peajesData => {
        const peajesIds = rutaPeajes[rutaKey];
        if (peajesIds) {
            return peajesIds.map(id => peajesData.find(p => p.id === id)).filter(Boolean) as typeof peajesData;
        }

        const peajesEncontrados: typeof peajesData = [];
        const radioKm = 15;
    ...
    return (
        <div className="space-y-6">
            <div>
                <h2 className="text-2xl font-bold text-gray-800 mb-2">Planificación de Rutas</h2>
                <p className="text-gray-600">
                    Calcula la ruta óptima para tus despachos considerando costos, peajes y tiempos de entrega.
                </p>
            </div>
